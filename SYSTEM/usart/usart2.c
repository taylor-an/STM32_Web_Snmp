...
static int usart4_send_str(void*,Usart_uint16);
...
//<<<<<<<<<<<<<<<<变量定义<<<<<<<<<<<<<<<<
static PSTUsart g_usart1_irq, g_usart2_irq, g_usart3_irq, g_usart4_irq, g_usart5_irq;
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

//<<<<<<<<<<<<<<<<函数定义<<<<<<<<<<<<<<<<

void mystrncpy(void * des, void * souce, u32 len)
{
...
//  
  ...
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_12;//USART1_TX
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
