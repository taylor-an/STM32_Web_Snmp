...
__align(4) ETH_DMADESCTypeDef  *DMAPTPRxDscrTab;
__align(4) ETH_DMADESCTypeDef  *DMAPTPTxDscrTab;
...
{ 
	...
	DMAPTPRxDscrTab  = mymalloc(SRAMIN, ETH_RXBUFNB*sizeof(ETH_DMADESCTypeDef));
	DMAPTPTxDscrTab  = mymalloc(SRAMIN, ETH_TXBUFNB*sizeof(ETH_DMADESCTypeDef));
	Rx_Buff=mymalloc(SRAMIN,ETH_RX_BUF_SIZE*ETH_RXBUFNB);	//申请内存
	Tx_Buff=mymalloc(SRAMIN,ETH_TX_BUF_SIZE*ETH_TXBUFNB);	//申请内存
	if(!DMARxDscrTab||!DMATxDscrTab||!Rx_Buff||!Tx_Buff||!DMAPTPRxDscrTab||!DMAPTPTxDscrTab)
	{
		ETH_Mem_Free();
		return 1;	//申请失败
	}	
...
{ 
	...
	myfree(SRAMIN,DMAPTPTxDscrTab);
	myfree(SRAMIN,DMAPTPRxDscrTab);
